...
public class ProcessUtils {
  ...
  public static void cancelApplication(List<String> appIds, Logger logger, String tenantCode,String workDir)
    ...
    if (appIds.size() > 0) {
      ...
      String cmd = "yarn application -kill " + appid;    //看这里，也是用cmd杀的；
      try {
  ...
  public static void killYarnJob(TaskInstance taskInstance) {
    try {
      ...
      String log = logClient.viewLog(taskInstance.getLogPath()); //查看日志，从日志里取 yarn app id
      if (StringUtils.isNotEmpty(log)) {
        ...
        if (appIds.size() > 0) {
          cancelApplication(appIds, logger, taskInstance.getProcessInstance().getTenantCode(), taskInstance.getExecutePath()); //这里用命令行杀
        }
      }
