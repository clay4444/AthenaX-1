...
public class ResInfo {
    ...
    public static Server parseHeartbeatForZKInfo(String heartBeatInfo){
        ...
        masterServer = new Server();   //这个Server代表最顶层的抽象
        masterServer.setHost(masterArray[0]);
        masterServer.setPort(Integer.parseInt(masterArray[1]));   //@TODO 不是进程id吗，怎么又变成端口号了？
        masterServer.setResInfo(getResInfoJson(Double.parseDouble(masterArray[2]),  //cpu和内存负载信息
                Double.parseDouble(masterArray[3]),
                Double.parseDouble(masterArray[4])));
        masterServer.setCreateTime(DateUtils.stringToDate(masterArray[5]));  //创建时间
        masterServer.setLastHeartbeatTime(DateUtils.stringToDate(masterArray[6])); //最后心跳时间
        return masterServer;
    }
