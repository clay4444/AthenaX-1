...
public class ProcessDao {
    ...
    private final int[] stateArray = new int[]{ExecutionStatus.SUBMITTED_SUCCESS.ordinal(),
            ExecutionStatus.RUNNING_EXEUTION.ordinal(),  //正在运行的
    ...
    /**
     * 处理一个Command，把这个Command转化为一个流程实例，比如master监听到了另一个master挂掉了，就会把挂掉的master上的流程实例的host地址置null
    ...
    private ProcessInstance constructProcessInstance(Command command, String host){
        ...
        if(cmdParam != null ){  //正常由quartz的Scheduler创建的话，cmdParam是null的
            ...
            if(cmdParam.containsKey(Constants.CMDPARAM_RECOVER_PROCESS_ID_STRING)) {  //包含 ProcessInstanceId，可能是说明这个流程实例是需要被容错的
                ...
                processInstanceId = Integer.parseInt(processId); //直接取出流程实例id
                if (processInstanceId == 0) {
                    logger.error("command parameter is error, [ ProcessInstanceId ] is 0");
                    return null;
                }
            }else if(cmdParam.containsKey(Constants.CMDPARAM_SUB_PROCESS)){  //processInstanceId  @TODO  ????? 什么意思
                // sub process map
                String pId = cmdParam.get(Constants.CMDPARAM_SUB_PROCESS);
                processInstanceId = Integer.parseInt(pId);
            }else if(cmdParam.containsKey(Constants.CMDPARAM_RECOVERY_WAITTING_THREAD)){  //WaittingThreadInstanceId
        ...
        if(command.getScheduleTime() != null){
            processInstance.setScheduleTime(command.getScheduleTime()); //流程实例的调度时间设置为command的调度时间
        }
        processInstance.setHost(host); //在当前master上调度的
        ...
        switch (commandType){
            case START_PROCESS:
                break;
            case START_FAILURE_TASK_PROCESS: //从失败的task节点开始运行任务
                // find failed tasks and init these tasks
                List<Integer> failedList = this.findTaskIdByInstanceState(processInstance.getId(), ExecutionStatus.FAILURE); //失败的
                List<Integer> toleranceList = this.findTaskIdByInstanceState(processInstance.getId(), ExecutionStatus.NEED_FAULT_TOLERANCE); //需要容错的
                List<Integer> killedList = this.findTaskIdByInstanceState(processInstance.getId(), ExecutionStatus.KILL); //被杀的
                cmdParam.remove(Constants.CMDPARAM_RECOVERY_START_NODE_STRING); //移除 StartNodeIdList

                failedList.addAll(killedList);
                failedList.addAll(toleranceList);
                for(Integer taskId : failedList){
                    initTaskInstance(this.findTaskInstanceById(taskId)); //重新初始状态
                }
                cmdParam.put(Constants.CMDPARAM_RECOVERY_START_NODE_STRING, //重新添加 StartNodeIdList
                        String.join(Constants.COMMA, convertIntListToString(failedList)));
                processInstance.setCommandParam(JSONUtils.toJson(cmdParam));
                processInstance.setRunTimes(runTime +1 );  //重试次数+1
                break;
            ...
            case RECOVER_SUSPENDED_PROCESS:
                ...
                break;
            case RECOVER_TOLERANCE_FAULT_PROCESS:   //容错master的时候用到了
                // recover tolerance fault process
                processInstance.setRecovery(Flag.YES);  //标记流程实例是失败恢复的
                runStatus = processInstance.getState(); //运行状态
                break;
    ...
    private void initTaskInstance(TaskInstance taskInstance){
        ...
        if(!taskInstance.isSubProcess()){  //不是子流程，
            if(taskInstance.getState().typeIsCancel() || taskInstance.getState().typeIsFailure()){ //任务是停止的或者失败的，意思就是正在运行的不管呗。。。。
                taskInstance.setFlag(Flag.NO);
                updateTaskInstance(taskInstance); //更新
                return;
            }
        }
        taskInstance.setState(ExecutionStatus.SUBMITTED_SUCCESS); //重新初始状态为提交成功
        updateTaskInstance(taskInstance);
    }
    ...
    /**
     * 处理需要容错的流程实例，把原流程实例的host(master地址)置空，然后往command表插入一条流程实例容错的命令
    ...
    public void processNeedFailoverProcessInstances(ProcessInstance processInstance){
        ...
        cmd.setCommandType(CommandType.RECOVER_TOLERANCE_FAULT_PROCESS); //命令类型，流程实例容错
        createCommand(cmd);
    }
